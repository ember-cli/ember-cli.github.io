...
### Vim
If you are using [Vim](http://www.vim.org/) with `ember-cli`, Vim creates temporary backups and autosaves which interfere with broccoli watcher, so they need to either be moved out of the way or disabled. To do that, ensure your .vimrc contains the following:
